...
Game.NPC = (function () {
  ...
  function createNPC(opts) {
    ...
    var npc = {
      ...
      sleepSpot: opts.sleepSpot || opts.home || { x: 0, y: 0 },
  ...
  function getSleepTargetPos(npc) {
    return npc.sleepSpot || npc.home;
  }

  function isAtHome(npc) {
    var homePos = getSleepTargetPos(npc);
    return U.distSq(npc.x, npc.y, homePos.x, homePos.y) < 22 * 22;
  }
  ...
  function assignResidentialHomes() {
    ...
    for (var i = 0; i < npcs.length; i++) {
      ...
      if (!best && candidates.length > 0) {
        // Always assign a house even if occupancy cap is exceeded
        best = candidates[0];
        var fallbackScore = Infinity;
        for (var fj = 0; fj < candidates.length; fj++) {
          var fh = candidates[fj];
          var fScore = U.distSq(npc.home.x, npc.home.y, fh.c.x, fh.c.y) + fh.occ * 8000;
          if (fScore < fallbackScore) { fallbackScore = fScore; best = fh; }
        }
      }

      if (best) {
        // Sleep inside the building (just behind the door) to avoid "sleeping outside" behavior
        var bedTx = best.b.doorX;
        var bedTy = Math.max(best.b.y + 1, best.b.doorY - 1);
        var sleepPos = { x: (bedTx + 0.5) * TS, y: (bedTy + 0.5) * TS };
        var doorPos = { x: (best.b.doorX + 0.5) * TS, y: (best.b.doorY + 0.5) * TS };

        npc.home = doorPos;
        npc.sleepSpot = sleepPos;
        best.occ++;
      }
    }
  }
    ...
    for (var i = 0; i < npcs.length; i++) {
      ...
      if (distToPlayer < NPC_UPDATE_RANGE) {
        ...
        if (npc.scheduledState === STATE.SLEEP) {
          var sleepTarget = getSleepTargetPos(npc);
          npc.x = U.lerp(npc.x, sleepTarget.x, 0.01);
          npc.y = U.lerp(npc.y, sleepTarget.y, 0.01);
  ...
  function updateNPCAI(npc, dt, hour, px, py, distToPlayer) {
    ...
    switch (npc.scheduledState) {
      case STATE.SLEEP:
        ...
        var sleepAt = getSleepTargetPos(npc);
        moveToward(npc, sleepAt.x, sleepAt.y, dt, 0.5);
        break;
      case STATE.WORK:
      ...
      case STATE.SOCIALIZE:
        ...
        updateVillageInteraction(npc, hour);
        break;
      case STATE.IDLE:
  ...
  function updateVillageInteraction(npc, hour) {
    if (npc.state !== STATE.SOCIALIZE || npc.barkTimer > 0) return;
    if (U.rng() > 0.01) return;

    var nearby = spatialHash.query(npc.x, npc.y, 56);
    var partner = null;
    for (var i = 0; i < nearby.length; i++) {
      var other = nearby[i];
      if (other.id === npc.id || !other.alive) continue;
      if (other.state !== STATE.SOCIALIZE || other.barkTimer > 0) continue;
      if (other.faction === 'bandits' || npc.faction === 'bandits') continue;
      partner = other;
      break;
    }
    if (!partner) return;

    var dx = partner.x - npc.x;
    var dy = partner.y - npc.y;
    npc.facing = Math.abs(dx) > Math.abs(dy) ? (dx > 0 ? 'E' : 'W') : (dy > 0 ? 'S' : 'N');
    partner.facing = npc.facing === 'E' ? 'W' : npc.facing === 'W' ? 'E' : npc.facing === 'N' ? 'S' : 'N';

    var lines = [
      'Evening to you.',
      'How fares your work today?',
      'The village feels alive tonight.',
      hour >= 18 ? 'See you at the tavern.' : 'Back to it after this.',
      npc.job === partner.job ? 'Another day in the same trade.' : 'Your craft keeps this place running.'
    ];
    setBark(npc, U.pick(lines));
    setTimeout(function (n, line) {
      if (n.alive && n.state === STATE.SOCIALIZE) setBark(n, line);
    }, 1200, partner, U.pick(['Aye.', 'True enough.', 'Well said.', 'Could not agree more.']));

    npc.wanderTimer = Math.max(npc.wanderTimer, 2.5);
    partner.wanderTimer = Math.max(partner.wanderTimer, 2.5);
    npc.playerRelation = U.clamp(npc.playerRelation + 0.1, -100, 100);
    partner.playerRelation = U.clamp(partner.playerRelation + 0.1, -100, 100);
  }
  ...
  function getActivityLabel(npc) {
    ...
    if (npc.state === STATE.SLEEP) return isAtHome(npc) ? 'Sleeping at home' : 'Going to bed';
    ...
    if (npc.state === STATE.SOCIALIZE) return 'Talking with villagers';
    if (npc.state === STATE.FIGHT) return 'In combat';
    if (npc.state === STATE.FLEE) return 'Fleeing';
    if (isAtHome(npc)) return 'At home';
